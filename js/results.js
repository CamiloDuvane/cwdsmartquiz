import { getQuizResults } from './quiz.js';

export async function viewAllResults() {
  ...
  try {
    ...
    const firebaseResults = await getQuizResults(currentUser.name);
    ...
    const sortedResults = firebaseResults.sort((a, b) => 
        ...
        ${sortedResults.map((result, index) => `
          <div class="result-card">
            ...
            <div class="result-details">
              <p>Data: ${new Date(result.timestamp).toLocaleDateString()} ${new Date(result.timestamp).toLocaleTimeString()}</p>
    ...
    // Initialize modal for showing result details
    initializeResultDetailsModal();
    
    // Make showResultDetails available globally
    ...
    alert('Erro ao carregar os resultados. Por favor, tente novamente.');
  }
}
...
function showResultDetailsModal(result) {
  ...
  modalContent.innerHTML = `
    <h3 class="modal-title">${getSubjectDisplayName(result.subject)} - ${new Date(result.timestamp).toLocaleDateString()}</h3>
